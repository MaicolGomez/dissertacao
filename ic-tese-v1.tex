...
So Horn's proposal was limited by the block size of the GPU device, which,
...
The basic idea is simple. The large array is divided into blocks, each of which
...
auxiliary buffer (as can see in \rfig{scheme} ~\ding{183}).  Lines $19$ to $25$
generate code for the runtime library to call the OpenCL driver to compile the
kernels and dispatch then for execution.

Lines $26$ executes the necessaries kernels to compute the scan
...
Back to \rlst{ClangScan}{d}, Line $32$ fillings a standard header. Lines $33$
...
Lines $37$ to $46$ define the neutral value according to the operation, a
...
Line $48$ defines the word $dataType$, it represents the type of variable that
is used for the programmer. Finally, line $49$ closes the file that will
...
Let vector $A$ and predicate refers to the elements greater than 10.\\
$A \: = \: <17, 4, 6, 8, 11, 5, 13, 19, 0, 24>$ and it is desired  $<17, 11, 13, 19, 24>$\\
As was mentioned before, the first step performs a vector of bits where
the $i$ element is $1$ if satisfies the predicate $0$ otherwise. For the example:\\
$input \:\: <17, 4, 6, 8, 11, 5, 13, 19, 0, 24>$\\
$bits \:\: <1, 0, 0, 0, 1, 0, 1, 1, 0, 1>$\\
Then scan a vector $bits$ into $bitsum$.\\
$bitsum \:\: <1, 1, 1, 1, 2, 2, 3, 4, 4, 5> $\\
The second step, to every element with value $1$ in the vector $bits$, it is saved the value from the vector input in the address that contains the element $i$ of the vector $bitsum$, after that, finally it is got the desire vector.\\
...
\rlst{strcod}  presents a fragment from $Stream Compaction$. The algorithm is based in the two steps mentioned before.
...
Due the version provided is a exclusive, it is necessary an additional step, so Line $21$ to pass from the exclusive to its inclusive version. Finally, Line $23$ fills the the vector output with the information generated in the vectors bitsum and bit.\\
\rlst{strker} presents the kernel generated by AClang. Due is a basic application of scan, the kernel generated has a basic structure. Line $3$ defines the neutral or also called identity element, in this case the operation used was a sum, thus the neutral value is $0$. As can be seen, each kernel has the type of variable for the vectors, this information was extracted from the omp clause and replaced as was explained in the section \ref{sec:ScanInAClang}.
Lines $6$ to $10$ presented the kernel that performs the steps ~\ding{182} and ~\ding{183} of Figure \ref{fig:scheme}. Lines $12$ to $16$ also presented the kernel that computes the step ~\ding{184} of Figure \ref{fig:scheme}. Finally, lines $18$ to $21$ show the kernel that performs the step ~\ding{185} of the same figure.
...
\item Flips bits of the predicate, and scan it.
