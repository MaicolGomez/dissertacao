...
that is required to render the graphics that are create and displayed by a computer.
Commonly, this requires the execution of the perform of the same operation on a large data set. Insomuch as that processing should be in real time. it mus be completed as
...
Generally each thread performs a operation on a sample data. This allows that the large data can be operated in parallel.
GPU was designed to graphics processing in mind this was the main application for GPUs
for a ling time. Lately, researcher and programmers discovered a opportunity the ability of the GPUs for high levels of parallelization. This is become an opportunity to increase the performance for many general purpose programs.
That is how started what is known as general purpose graphics processor unit programming or GPGPU programming. With the steady growth interest in GPGPUs programming, GPU vendors started creating GPUs for different applications in mind,thus, toolkit was created to allow program in the GPUs. This toolkit is known as CUDA, that allows to the programmer creates applications to can run on the GPU, a big advantage is the extremely similar language between CUDA and C.
This allows the implementation of many applications as well could increased the number of parallel algorithms taking advantage of the potentially of the GPU. As was mentioned before, GPUs have the ability to execute a program in parallel. The downside to using GPU is the fact that a CPU is necessary mandatory. The GPU by itself can't be a standalone unit. To be able to operate on a GPU, the presence of the CPU is necessary to manage the execution of the program. The CPU is responsible for determining which portions of the application are completed by the GPU and what the parameters will be used in this operation. Also, an important function of the CPU is responsible for the memory management of the GPU. Thereby the operations are performed in the GPU, the data must be copied from the CPU memory. In the same way, when the GPU finished its work, it is necessary pass the data from the GPU to the CPU. This is a very expensive operation that often limits the programmers to be able to use the GPU.
...
For these reasons it is very important to define what portions of the program can be done serially and executed on the CPU meanwhile what portions could be parallelized and executed on the GPU. If the proportion to be executed in the GPU compensates in time with all the disadvantages mentioned above and it is more faster that its serial version, worth the use of the GPU.
Many studies have been done about GPUs in recent years as researchers look into how a wide range of applications fare on the GPU. The new ease of use has also contributed to an increase in the number of people wanting to conduct research into the viability of a GPU implementation for their application. Studies also look at the various ways that the GPU can be utilized to improve performance. In these days, a very common situation, it is that of many people waiting for some research that makes the use of the GPU lets viable the GPU use in their application.\\
An example is study ~\cite{Trancoso2005} which looks at a very simple application as implemented on a GPU, a low end CPU, and a high end CPU. It discusses the application’s performance on the GPU relative to the CPU and also looks at several of the different variables that can be changed to improve the GPU’s performance. It also looks at what factors make a program more likely to be better suited for a GPU than a CPU. Another study ~\cite{Owens2008}, is more an example of a study that just looks at how the GPU can handle applications that were previously implemented on a CPU. This study looks at the GPU design and discusses the possible performance improvements offered for the GPU. It then looks at how the GPU did for some specific applications such as in-game, physics and computational biophysics.
...
Before going into programming of GPUs \ref{sec:gpuprogramming}, it is important to have some background.
