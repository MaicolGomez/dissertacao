...
\rem{the ability of}\new{to explore} the GPUs \rem{for}\new{to achieve} high
levels of parallelization. This \rem{is becomes}\new{became} an opportunity to
increase the performance of many general purpose programs.  That is how started
what is known as general purpose graphics processing unit programming or GPGPU
...
$x_{k}$. \rem{In Figure \ref{fig:scanhillis}, this process is
illustrated}\new{\rfig{scanhillis} shows this process} for an array of 8
...
In  the down-sweep  phase,  the  tree is  traversed
...
In 2005, Horn~\cite{GPUGems2} proposed a first version of scan parallel for
...
So the proposed of Horn was limited by the block size of the GPU device, in our
...
The basic idea is simple. It is divided the large array into blocks, each of
...
increments that that are added to all elements in their respective blocks. For
example (see~\rfig{largescan}), let $N$ be the number of elements in
...
scan clause, as detailed in \rlst{ClangScan}{a}, lines 3 through 20. For
instance, Lines 3 to 7 get the list of variables and the kind of operation
associated with the scan clause.

\marcio{Este parágrafo está muito confuso. Além disso, o código da alinea b na
listagem não diz absolutamente nada. Ninguem da banca irá entender. Ou vc
remove e simplifica o texto aqui ou coloca código que faça algum sentido.} In
\rlst{ClangScan}{b}, it's recovered the buffer where the result of the scan
algorithm will be written, as can be seen in \rlstn{ScanKind}{a}{b}.  In
\rlst{ScanKind}{a} is a standard way to write scan algorithm (input and output
vectors). In \rlst{ScanKind}{b} is showed a second way to write scan algorithm,
...
$CGM.operation1.MPtoGPURuntime().operation2$. Lines 24 to 34 retrieve the scan
parameters and and makes an initial configuration of OpenCL. Line $24$ computes
...
Line $28$ does a needed step to execute the scan algorithm, it created an
auxiliary buffer (as can see in \rfig{scheme} ~\ding{183}).  Lines $29$ to $33$
...
independently (as can see in \rfig{scheme} ~\ding{182}). Line $32$ executes
the second kernel that is in charge to computed the sum of accumulates from the
auxiliary vector (as seen in \rfig{scheme} ~\ding{184}). Line $33$ executes
