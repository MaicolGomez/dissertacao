...
By the mid-1990s, the demand \rem{of}\new{for} applications that were using 3D graphics
increases considerably growing one stage of development. PC gaming was affected
\rem{for}\new{by} those devices to create progressively more realistic 3D environments.
...
that \rem{is}\new{are} required to render the graphics that are create\new{d} and displayed by a computer.
Commonly, this requires the execution of \rem{the perform of} the same operation on a large data set. Insomuch as that processing should be in real time. it must be completed as
...
Generally, each thread performs a\new{n} operation on a sample data. This allows that the large data can be operated in parallel.
GPU was designed \rem{to}\new{for} graphics processing in mind, \new{and} this was the main application for GPUs
for a long time. Lately, researcher and programmers discovered a opportunity the ability of the GPUs for high levels of parallelization. This \rem{is} become\new{s} an opportunity to increase the performance \rem{for}\new{of} many general purpose programs.
That is how started what is known as general purpose graphics \rem{processor}\new{processing} unit programming or GPGPU programming. With the steady growth interest in GPGPUs programming, GPU vendors started creating GPUs for different applications in mind, thus, \new{the} toolkit was created to allow program\new{ming} in the GPUs. \rem{This toolkit is known as CUDA, that allows to the programmer creates applications to can run on the GPU, a big advantage is the extremely similar language between CUDA and C.
This allows the implementation of many applications as well could increased the number of parallel algorithms taking advantage of the potentially of the GPU}\new{This toolkit, known as CUDA, allows the programmer to create applications that can run on GPUs. A major advantage of CUDA is its similarity with the C language. This allows the implementation of many applications as well as the increase in the number of parallel algorithms to harness the potential of the GPU}. As was mentioned before, GPUs have the ability to execute a program in parallel. The downside to using GPU is the fact that a CPU is necessary mandatory. The GPU by itself can't be a standalone unit. To be able to operate on a GPU, the presence of the CPU is necessary to manage the execution of the program. The CPU is responsible for determining which portions of the application are completed by the GPU and what the parameters will be used in this operation. Also, an important function of the CPU is responsible for the memory management of the GPU. Thereby the operations are performed in the GPU, the data must be copied from the CPU memory. In the same way, when the GPU finished its work, it is necessary pass the data from the GPU to the CPU. This is a very expensive operation that often limits the programmers to be able to use the GPU.
...
For these reasons, it is very important to define what portions of the program can be done serially and executed on the CPU, meanwhile what portions could be parallelized and executed on the GPU. If the proportion to be executed in the GPU compensates in time with all the disadvantages mentioned above and it is \rem{more} faster \rem{that}\new{than} its serial version, worth the use of the GPU.
\marcio{Não entendi esta frase:}\ed{Many studies have been done about GPUs in recent years as researchers look into how a wide range of applications fares on the GPU.} The new ease of use has also contributed to an increase in the number of people wanting to conduct research into the viability of a GPU implementation for their application. Studies also look at the various ways that the GPU can be utilized to improve performance. \rem{In these days, a very common situation, it is that of many people waiting for some research that makes the use of the GPU lets viable the GPU use in their application.}\new{A very common situation nowadays is the waiting of many people for some research that makes possible the use of GPUs in their applications.}

\rem{An example is study ~\cite{Trancoso2005} which looks at a very simple application as implemented on a GPU, a low end CPU, and a high end CPU. It discusses the application’s performance on the GPU relative to the CPU and also looks at several of the different variables that can be changed to improve the GPU’s performance. It also looks at what factors make a program more likely to be better suited for a GPU than a CPU.}\new{As an example, Trancoso \etal~\cite{Trancoso2005} analysed a very simple application as implemented on a GPU, a low-end CPU, and a high-end CPU. They discussed the application’s performance on the GPU relative to the CPU and also looked at several of the different variables that can be changed to improve the GPU’s performance. They also looked at what factors make a program more likely to be better suited for a GPU than a CPU.}

\marcio{Melhorar como no exemplo acima.} Another study ~\cite{Owens2008}, is more an example of a study that just looks at how the GPU can handle applications that were previously implemented on a CPU. This study looks at the GPU design and discusses the possible performance improvements offered for the GPU. It then looks at how the GPU did for some specific applications such as in-game, physics and computational biophysics.
...
Before going into programming of GPUs \new{(see~\rsec{gpuprogramming})}\rem{\ref{sec:gpuprogramming}}, it is important to have some background.\marcio{background em que? Na arquitetura das GPUs? É preciso explicitar}
...
In the 80's was developed an approach to solve scan algorithm in parallel.
Hillis and Stelle~\cite{dataparallel} presented the following algorithm.

\begin{figure}[t]
	\centering
	\caption{Parallel scan in $\mathcal{O}(nlog\  n)$}
	\subfloat[Hillis and  Steele algorithm ~\cite{harris2007parallel} ]{
		\includegraphics[scale=1.0]{images/hillis.jpg}
		\label{fig:hillis}
	}
\end{figure}
...
\begin{figure}[t]
	...
	\subfloat[Up-sweep phase~\cite{harris2007parallel}]{
		\includegraphics[scale=0.35]{images/upsweep.pdf} \label{fig:upsweep}
	}
	\vfill
	\subfloat[Down-sweep phase~\cite{harris2007parallel}]{
...
Lines 3 to 7 gets the list of variables and the kind of operation
