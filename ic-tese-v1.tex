...
\guido{Make the writing for AClang uniform. I found Aclang, ACLang, etc.; it should be AClang}
...
\begin{figure}[p]
	...
	\includegraphics[scale=0.50]{images/thread_organization.png}
...
\ed{One of the algorithm proposed by Hillis and Stelle~\cite{dataparallel} aimed at  performing the sum of an array of numbers (see \rfig{reductionhillis}), an operation which nowadays is calleed  $reduction$.\\ The main idea behind their work was to organize the array data at the leaves of a
...
\guido{The key to understand this algorithm is to remember, in defining
...
that determines whether a processor will perform the assignment (line $5$ to $14$). This
...
\rfig{scanhillis} the processor $k = 1$ stors the sum of the neighbors at
...
performs operations in parallel, i.e. in constant time.
...
\guido{IMPORTANT}
\guido{I changed this following paragraph in a way that might changed the meaning; check if the meaning is still correct}
\ed{\rlst{TemplateScan}{a} defineds the header for each kernel. This example shows
...
between two variables. It specifies the  new operation defined for a new
variable type. It is possible to \guido{Operate what? what are you doing, very unclear!!} operate two variable in a simple way for a
primitive variables: for example for two variables $x$ and $y$ of type $int$ we
use the line $10$ to compute the result.}
...
\guido{IMPORTANT}
\guido{I changed this following paragraph in a way that might changed the meaning; check if the meaning is still correct}
\ed{Back to \rlst{ClangScan}{d}, Line $32$ fills a standard header. Lines $33$
...
Since the version provided is exclusive, it is necessary an additional step (Line $21$) to pass from the exclusive to the inclusive version. Finally, Line $23$ fills the the vector output with the information generated in the vectors $bitsum$ and $bits$.
...
\begin{enumerate}
\item \ed{Generate a vector of the list (bit in common, starting from the least significant bit)
where every bit that is 0 in the new vector is 1 else the value is 0.}
\item \ed{Scan the vector, and record the sum of the predicate in the process. Notice, the scan
algorithm works for arrays of arbitrary sizes instead of $2^{n}$ sizes; however as 
explained before} \guido{the proposed what? approach} the proposed \ed{works}  for any arbitrary size.
...
in the step 1, and also generates the opposite vector \guido{of the first one? what is this? Not clear}  of the first one. 
...
mentioned before. Finally, lines (27 -- 32) move the elements in accordance to the vectors generated in the previous step.}
